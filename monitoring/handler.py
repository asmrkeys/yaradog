...
class ChangeHandler(FileSystemEventHandler):
    ...
    async def log_event(self, event_type, event_path, dest_path=None):
        ...
        if not event_path.startswith(self.scan_whitelist_tuple):
            if yara_scan(event_path)[0]:
                if event_path.lower() != self.yara_forge_rules_full_path.lower():
                    ...
                    if self.delete_malware:
                        await self.delete_file(event_time, event_path)
                    await session_log(f'[{event_time}] INFO: Alert received by {yara_rule} YARA Rule in "{event_path}"')
            else:
                if event_type == 'File created':
                    if event_path.endswith(self.file_extensions_tuple):
    ...
    async def delete_file(self, event_time, event_path):
